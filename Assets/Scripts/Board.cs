...
public class Board
{
    ...
    public void UpdateBoard(int col, bool isPlayer)
    {
        ...
        playerBoard[updatePos][col] =
            isPlayer ? PlayerType.RED : PlayerType.BLUE;

        currentPos = new GridPos
        {
            row = updatePos,
            col = col
        };
    ...
    public bool Result(bool isPlayer)
    {
        ...
        return IsHorizontal(current)
            || IsVertical(current)
            || IsDiagonal(current)
            || IsReverseDiagonal(current);
    }
    ...
    {
        List<GridPos> resultList = new() { start };
    ...
    bool SearchResult(List<GridPos> searchList, PlayerType current)
    {
        ...
        {
            PlayerType compare =
                playerBoard[searchList[i].row][searchList[i].col];
